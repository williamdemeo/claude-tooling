...
Source of truth: the williamdemeo/claude-tooling repo (`projects/agda-native-air/`), symlinked into place at `~/git/formalverification/agda-native-air/CLAUDE.md` and `~/git/formalverification/agda-native-air/.claude/`.  Add or edit skills there, then run `make install PROJECT=agda-native-air` in claude-tooling; project skills belong in that repo, not in `~/.claude/skills/` and not as committed files in this repository.

PROBE-MARKER: claude-tooling/agda-native-air
