#!/bin/bash
# SessionStart hook: this is for provisioning the Nix toolchain so Agda
# typechecking works in Claude Code sessions on the web or launched
# from a terminal **outside** of a Nix shell.  If the user has launched
# Claude Code from inside a Nix shell, this hook is skipped (the shell
# already has the right Agda).
#
# The Agda code typechecks only through its Nix flake (flake.nix pins Agda and the
# Agda libraries); there is no system `agda` unless this session was launched from
# inside a Nix shell.
#
